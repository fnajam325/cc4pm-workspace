...
PRDs are archived like the competitive scans in `research/competitive-scans/` and the status updates in `docs/status-updates/`: one permanent file per revision, never overwritten.

1. Write the new/updated PRD to `docs/prds/YYYY-MM-DD.md` (today's date). If a PRD revision already happened today, update that same file rather than creating a second one for the same day.
2. Overwrite `docs/prds/latest.md` with the current full contents, plus a one-line header noting the run date and linking to the dated file and `README.md`.
3. Add or update today's row in `docs/prds/README.md` (date, one-line headline describing what changed in this revision, link to the dated file).
4. Follow this project's standard tracking convention: propose adding/confirming `docs/prds/latest.md` and `README.md` in `CLAUDE.md`'s Project Files index (only needed once) and logging the change in `change_log.md` (decision, rationale, owner) — don't skip the audit trail just because this is "just a PRD."
